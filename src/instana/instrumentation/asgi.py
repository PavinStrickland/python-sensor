...
class InstanaASGIMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if isinstance(request_headers, list):
            request_context = async_tracer.extract(
                opentracing.Format.BINARY, request_headers
            )
                ...
                if response["type"] == "http.response.start":
                    try:
                        ...
                        if headers is not None:
                            ...
                            async_tracer.inject(
                                span.context, opentracing.Format.BINARY, headers
                            )
                    except Exception:
        ...
        with async_tracer.start_active_span(
            "asgi", child_of=request_context
        ) as tracing_scope:
