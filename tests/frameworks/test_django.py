...
class TestDjango(StaticLiveServerTestCase):
    ...
    def test_request_with_error(self):
        ...
        self.assertEqual(
            "This is a fake error: /cause-error", django_span.data["http"]["error"]
        )
    ...
    def test_response_header_capture(self):
        ...
        self.assertEqual(200, django_span.data["http"]["status"])
        self.assertEqual(
            "^response_with_headers$", django_span.data["http"]["path_tpl"]
        )

        self.assertIn("X-Capture-This-Too", django_span.data["http"]["header"])
        self.assertEqual(
            "this too", django_span.data["http"]["header"]["X-Capture-This-Too"]
        )
        self.assertIn("X-Capture-That-Too", django_span.data["http"]["header"])
        self.assertEqual(
            "that too", django_span.data["http"]["header"]["X-Capture-That-Too"]
        )
