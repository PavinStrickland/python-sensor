...
class TestOTSpan(unittest.TestCase):
    ...
    @unittest.skipIf(
        sys.version_info >= (3, 11), reason="Raises not Implemented exception in OSX"
    )
    ...
    def test_tag_values(self):
        ...
        self.assertEqual(
            test_span.data["sdk"]["custom"]["tags"]["uuid"],
        ...
        self.assertListEqual(
            test_span.data["sdk"]["custom"]["tags"]["mylist"], [1, 2, 3]
        )
    ...
    def test_tag_names(self):
        with tracer.start_active_span("test") as scope:
            ...
            # Unicode key name
            scope.span.set_tag("asdf", "This should be ok")
        ...
        self.assertEqual(
            test_span.data["sdk"]["custom"]["tags"]["asdf"], "This should be ok"
        )
    ...
    def test_custom_service_name(self):
        ...
        with tracer.start_active_span("entry_span") as scope:
            ...
            scope.span.set_tag("type", "entry_span")

            with tracer.start_active_span(
                "intermediate_span", child_of=scope.span
            ) as exit_scope:
                exit_scope.span.set_tag("type", "intermediate_span")

            with tracer.start_active_span(
                "exit_span", child_of=scope.span
            ) as exit_scope:
                exit_scope.span.set_tag("span.kind", "client")
                exit_scope.span.set_tag("type", "exit_span")

        spans = tracer.recorder.queued_spans()
        self.assertEqual(len(spans), 3)

        filter = (
            lambda span: span.n == "sdk" and span.data["sdk"]["name"] == "entry_span"
        )
        entry_span = get_first_span_by_filter(spans, filter)
        self.assertTrue(entry_span)

        filter = (
            lambda span: span.n == "sdk"
            and span.data["sdk"]["name"] == "intermediate_span"
        )
        intermediate_span = get_first_span_by_filter(spans, filter)
        self.assertTrue(intermediate_span)

        filter = (
            lambda span: span.n == "sdk" and span.data["sdk"]["name"] == "exit_span"
        )
