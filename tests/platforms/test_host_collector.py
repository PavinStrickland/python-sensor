...
class TestHostCollector(unittest.TestCase):
    ...
    def test_prepare_payload_basics(self):
        ...
        self.assertIn(
            type(python_plugin["data"]["metrics"]["ru_inblock"]), [float, int]
        )
        self.assertIn("ru_oublock", python_plugin["data"]["metrics"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["ru_oublock"]), [float, int]
        )
        ...
        self.assertIn(
            type(python_plugin["data"]["metrics"]["ru_nsignals"]), [float, int]
        )
        ...
        self.assertIn(
            type(python_plugin["data"]["metrics"]["alive_threads"]), [float, int]
        )
        self.assertIn("dummy_threads", python_plugin["data"]["metrics"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["dummy_threads"]), [float, int]
        )
        self.assertIn("daemon_threads", python_plugin["data"]["metrics"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["daemon_threads"]), [float, int]
        )
        ...
        self.assertIn(
            type(python_plugin["data"]["metrics"]["gc"]["collect0"]), [float, int]
        )
        self.assertIn("collect1", python_plugin["data"]["metrics"]["gc"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["gc"]["collect1"]), [float, int]
        )
        self.assertIn("collect2", python_plugin["data"]["metrics"]["gc"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["gc"]["collect2"]), [float, int]
        )
        self.assertIn("threshold0", python_plugin["data"]["metrics"]["gc"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["gc"]["threshold0"]), [float, int]
        )
        self.assertIn("threshold1", python_plugin["data"]["metrics"]["gc"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["gc"]["threshold1"]), [float, int]
        )
        self.assertIn("threshold2", python_plugin["data"]["metrics"]["gc"])
        self.assertIn(
            type(python_plugin["data"]["metrics"]["gc"]["threshold2"]), [float, int]
        )
    ...
    def test_prepare_payload_with_snapshot_with_python_packages(
        self, mock_should_send_snapshot_data
    ):
    ...
    @patch.object(HostCollector, "should_send_snapshot_data")
    def test_prepare_payload_with_snapshot_disabled_python_packages(
        self, mock_should_send_snapshot_data
    ):
        mock_should_send_snapshot_data.return_value = True
